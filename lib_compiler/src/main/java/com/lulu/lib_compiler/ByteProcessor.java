...
public class ByteProcessor extends AbstractProcessor {
    private static final String TAG = "ByteProcessor";
    ...
    private void handleByteService(RoundEnvironment roundEnvironment) {
        ...
        if (sets != null && sets.size() > 0) {
            println("将要处理 ByteService 注解：" + sets);
            ...
            for (Element element : sets) {
                ...
                if (element instanceof TypeElement) {
                    ...
                    println("保存：Interface: " + interName + " Impl: " + implName);
                }
            }
            //生成代码
            generate();
            println("总耗时：" + (System.currentTimeMillis()-time)/1000f+ " s");
        }
    }
    ...
    private void handleBindView(RoundEnvironment roundEnvironment) {
        ...
        for (Element element : sets) {
            ...
            ClassName baseViewBindItemClassName = ClassName.get("com.lulu.annotationprocessordemo", "BaseViewBindItem");

            MethodSpec cons = MethodSpec.constructorBuilder()
                    .addModifiers(Modifier.PUBLIC)
                    .addParameter(baseViewBindItemClassName, "viewBindItem")
                    .addStatement("this.viewBindItem = viewBindItem")
//                    .addStatement("viewBindItem." + filedName + "")
                    .addStatement("viewModelMap.put(" + value + ", " + "viewBindItem." + filedName + ")")
                    .build();

            //viewModelMap
            ParameterizedTypeName mapType = ParameterizedTypeName.get(ClassName.get(Map.class),
                    ClassName.get(Integer.class), ClassName.get("com.lulu.annotationprocessordemo","IModel"));
            ...
            //class
            TypeSpec typeSpec = TypeSpec.classBuilder(className + "_ProviderViewBindModel")
                    .addModifiers(Modifier.PUBLIC, Modifier.FINAL)
                    .addField(map)
